...
void lineEditor::newText() //Creates a fresh page and clears the vector.
{
    ...
    std::cout << "***********************************************************************************" << endl;
    std::cout << "!N = Blank Page | !E = Edit | !D = Delete Line | !O = Open | !S = Save | !Q to quit" << endl;
    std::cout << "***********************************************************************************" << endl;
    linenum = 1;
    std::cout << linenum << "> ";
}
...
void lineEditor::deleteText() 
{
    int lineDel;    

    //User defined position to delete
    std::cout << std::endl << "Enter the line number you wish to delete: ";
    std::cin >> lineDel;
    if(lineDel == 0 || lineDel == " ") { std::cout << "Bad Number" << std::endl; exit(1); } //Input verification
    lineDel = lineDel - lnr;

    //Delete line
    text.erase(text.begin()+(lineDel), text.begin()+(lineDel+1));
