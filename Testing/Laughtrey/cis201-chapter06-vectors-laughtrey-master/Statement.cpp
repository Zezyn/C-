...
void Statement::print() // prints the statement, prints daily balance and finally the min/average interest
{
   ...
   {
      if (i == transactions[index].get_day())
      {
         std::cout << "Day: " << transactions[index].get_day() << std::endl; // Getting Seg Fault Error here.
         std::cout << "Descp: " << transactions[index].get_description() << std::endl;
