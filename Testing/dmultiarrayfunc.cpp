...
int main() {
    ...
    int cstation;
    int lab;
    string labs[TOTAL_LABS] = { 5,6,4,3 };
...
/*
    for(int row = 0; row<TOTAL_LABS;row ++) {
		for(int col=0; col<sizeof(labs[row])/sizeof(labs[row][col]); col++) {
			labs[row][col] = "Empty";
		}
    }	
}
...
void printarrays(string &id, int &cstation, int &lab, string labs[][TOTAL_STATIONS]) {
	...
	for(int rows=0 ;rows<TOTAL_LABS;rows++) {
		cout << " " << (rows+1) << ":    ";
	    for(int cols=0; cols<sizeof(labs[rows])/sizeof(labs[rows][cols]);cols++) {
			cout << (cols+1) << ": ";
	        cout << labs[rows][cols] << "     ";
        }	
	cout << endl;
    }
	cout << "SIZE OF: " << sizeof(labs[1])/sizeof(labs[0][1]) << endl;
	menu(id, cstation, lab, labs);
}
